...
employer::employer(int g,int h,int i,string a,string b,string c,string d,long e,long f,string j,float k,bool w):personne(g,h,i,a,b,c,d,e,f)
{
		grade=j;
		salaire=k;
		embauche=w;
...
void employer::remplir_employer(vector<employer*> temployer)
{
	embauche=true;
	...
	while(exist_carte_identitee(temployer,carte_d_identitee)!=-1)
	{
	cout<<"donner carte d'identitee : ";cin>>carte_d_identitee;
	}
	...
	string j;
	cout<<"donner grade : ";cin>>j;
	while(j!="Pilote"&&j!="Pilote assistant"&&j!="Stewart"&&j!="Hautesse"&&j!="Ingénieur aéronautique")
	{cout<<"choisir le grade de l'employer svp:"<<endl<<"+Pilote  +Pilote assistant  +Stewart  +Hautesse  +Ingénieur aéronautique"<<endl;
	cin>>j;}
	grade=j;
	cout<<"donner salaire : ";cin>>salaire;
};
int employer::exist_carte_identitee(vector<employer*> t,long a)
{
...
int existe_carte_id(vector<employer*> t,long a)
{
	for(unsigned int i=0;i<t.size();i++)
		if(t[i]->get_carte_id()==a)
			return(i);
...
istream& operator>> (istream & in,employer & p)
	...
	in>>p.date_de_naissance;in.ignore();
	in>>p.embauche;in>>p.grade;in>>p.salaire;
	for(unsigned int i=0;i<31;i++)
		for(unsigned int j=0;j<13;j++)
...
void employer::modifier_employer(vector<employer*> t)
{
	int y=1;
	while(y!=0)
	{cout<<" 1 => modifier nom"<<endl<<" 2 => modifier prenom"<<endl<<" 3 => modifier adress"<<endl<<" 4 => modifier email"<<endl<<" 5 => modifier carte d'identitee"<<endl<<" 6 => modifier  carte bancaire"<<endl<<" 7 => modifier date de naissance"<<endl<<" 8 => modifier grade"<<endl<<" 9 => modifier salaire"<<endl<<" 0 => Quitter"<<endl;
		...
		case 5:{cout<<"donner carte d'identitee : ";cin>>carte_d_identitee;
				while(existe_carte_id(t,carte_d_identitee)==-1)
				{cout<<"donner carte d'identitee : ";cin>>carte_d_identitee;
				}
				break;}
		...
		case 8:{cout<<"donne grade : ";cin>>grade;break;}
		case 9:{cout<<"donne salaire : ";cin>>salaire;break;}
		}
	}
};
